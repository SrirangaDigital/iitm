...
<?php foreach ($data as $row) { ?>
        <div class="post">
            <a href="<?=BASE_URL?>listing/photos/<?=$row?>" title="View Album">
                <div class="fixOverlayDiv">
                    <img class="img-responsive" src="<?=$viewHelper->includeRandomThumbnail($row)?>">
                    <div class="OverlayText"><?=$viewHelper->getPhotoCount($row)?><br /><small><?=$viewHelper->getDetailByFieldForAlbum($row, 'Event')?></small> <span class="link"><i class="fa fa-link"></i></span></div>
                </div>
                <p class="image-desc">
